<?php
/**
 * Mastery Score Updater Utility
 * Handles automatic updating of user Mastery Score in the database
 */
...
function updateUserGWA($pdo, $user_id, $game_type) {
    ...
    // Calculate Mastery Score using the same formula as in profile.php
    $gwa = $result['total_level'] * 1.5;
    error_log("Calculated Mastery Score: $gwa (from total_level: {$result['total_level']})");
    ...
    try {
        ...
        if ($currentGWA) {
            error_log("Existing Mastery Score record found. Current Score: " . $currentGWA['current_gwa']);
        } else {
