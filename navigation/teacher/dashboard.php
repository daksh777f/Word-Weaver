...
// Total learner count
$stmt = $pdo->query("SELECT COUNT(*) as count FROM users WHERE grade_level NOT IN ('Teacher', 'Creator', 'Admin', 'Developer')");
$total_students = $stmt->fetch()['count'];

// Active learners (logged in within the last 30 minutes)
$active_students = 0;
try {
    $stmt = $pdo->prepare("
        ...
        AND u.grade_level NOT IN ('Teacher', 'Creator', 'Admin', 'Developer')
    ");
    $stmt->execute();
    $result = $stmt->fetch();
    if ($result) {
...
// Audience segments count
$stmt = $pdo->query("SELECT COUNT(DISTINCT grade_level) as count FROM users WHERE grade_level NOT IN ('Teacher', 'Creator', 'Admin', 'Developer')");
...
<head>
    ...
    <title>Creator Studio - Word Weavers</title>
