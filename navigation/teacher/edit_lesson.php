...
// Check if user is logged in and has creator/admin access
$gradeLevel = $_SESSION['grade_level'] ?? '';
$isTeacherOrAdmin = canAccessCreatorConsole($gradeLevel);
...
<head>
    ...
    <title>Edit Lesson - Word Weavers</title>
        ...
        <div class="teacher-container">
            <div class="vocabulary-section">
                <div class="section-header" style="justify-content: flex-start; gap: 1rem; flex-direction: row; align-items: center;">
                    ...
                    <h3 style="margin: 0; font-size: 1rem; font-family: 'Press Start 2P', cursive; line-height: 1.5;">Edit Lesson</h3>
                </div>
                
                <form id="editLessonForm" onsubmit="saveLesson(event)" style="background: rgba(255,255,255,0.05); padding: 2rem; border-radius: 12px; border: 1px solid rgba(255,255,255,0.1);">
                    <input type="hidden" id="lessonId" name="id" value="<?= $lesson['id'] ?>">
                    ...
                    <div class="form-group" style="margin-bottom: 1.5rem;">
                        ...
                        <input type="text" id="title" name="title" required value="<?= htmlspecialchars($lesson['title']) ?>" style="width: 100%; padding: 0.8rem; border-radius: 8px; border: 1px solid rgba(255,255,255,0.2); background: rgba(0,0,0,0.2); color: white;">
                    </div>

                    <div class="form-row" style="display: flex; gap: 20px; margin-bottom: 1.5rem;">
                        <div class="form-group" style="flex: 1;">
                            <label for="gradeLevel" style="display: block; color: var(--text-secondary); margin-bottom: 0.5rem;">Target Audience <span style="color: #ff6b6b;">*</span></label>
                            <select id="gradeLevel" name="grade_level" required style="width: 100%; padding: 0.8rem; border-radius: 8px; border: 1px solid rgba(255,255,255,0.2); background: rgba(0,0,0,0.2); color: white;">
                                ...
                                <option value="Beginner" <?= $lesson['grade_level'] == 'Beginner' ? 'selected' : '' ?>>Beginner (Foundation)</option>
                                <option value="Intermediate" <?= $lesson['grade_level'] == 'Intermediate' ? 'selected' : '' ?>>Intermediate (Developing)</option>
                                <option value="Advanced" <?= $lesson['grade_level'] == 'Advanced' ? 'selected' : '' ?>>Advanced (Proficient)</option>
                                <option value="Expert" <?= $lesson['grade_level'] == 'Expert' ? 'selected' : '' ?>>Expert (Mastery)</option>
                            </select>
                        </div>
                        <div class="form-group" style="flex: 1;">
                            ...
                            <select id="section" name="section" style="width: 100%; padding: 0.8rem; border-radius: 8px; border: 1px solid rgba(255,255,255,0.2); background: rgba(0,0,0,0.2); color: white;">
                                ...
                                <?php foreach ($sections as $sec): ?>
                                    <option value="<?= htmlspecialchars($sec) ?>" <?= $lesson['section'] == $sec ? 'selected' : '' ?>><?= htmlspecialchars($sec) ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-group" style="margin-bottom: 2rem;">
                        ...
                        <div style="position: relative; min-height: 300px;">
                            ...
                            <textarea id="lessonContent" name="content"><?= htmlspecialchars($lesson['content']) ?></textarea>
                        </div>
                    </div>
                    ...
                    <div class="form-actions" style="display: flex; gap: 10px; justify-content: flex-end;">
                        ...
                        <button type="submit" style="padding: 0.8rem 1.5rem; border-radius: 8px; background: #10b981; border: none; color: white; cursor: pointer; font-weight: 600;">Save Changes</button>
                    </div>
                </form>
            </div>
        </div>
