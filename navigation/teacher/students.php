...
// Build the query for learners (exclude creator/admin roles)
$query = "SELECT id, username, email, grade_level, section, created_at FROM users WHERE grade_level NOT IN ('Teacher', 'Creator', 'Admin', 'Developer')";
$params = [];
...
// Get unique learner segments for filter
$grade_levels = $pdo->query("SELECT DISTINCT grade_level FROM users WHERE grade_level NOT IN ('Teacher', 'Creator', 'Admin', 'Developer') ORDER BY grade_level")->fetchAll(PDO::FETCH_COLUMN);
...
<head>
    ...
    <title>Students - CodeDungeon</title>
