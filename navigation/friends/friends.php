...
foreach ($friends_data as $friend_data) {
    ...
    $is_jaderby = in_array(strtolower($friend_data['username']), ['daksh goel', 'chirag aggarwal'], true);
...
foreach ($all_users as $suggested_user) {
    ...
    // Determine the highest badge for this user
    $is_jaderby = in_array(strtolower($suggested_user['username']), ['daksh goel', 'chirag aggarwal'], true);
...
<head>
    ...
    <title>Friends - CodeDungeon</title>
    ...
    <link rel="stylesheet" href="friends.css?v=<?php echo filemtime('friends.css'); ?>">
